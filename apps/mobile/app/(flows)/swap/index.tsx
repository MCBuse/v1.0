...
export default function SwapScreen() {
  ...
  const [executeResult, setExecuteResult] = useState<{ fromAmount: string; toAmount: string; toCurrency: string } | null>(null);
  ...
  useEffect(() => {
    ...
    preview.mutate({ fromCurrency: from, toCurrency: to, fromAmount: baseUnits });
  ...
  const handleSwap = useCallback(async () => {
    ...
    try {
      const result = await execute.mutateAsync({ fromCurrency: from, toCurrency: to, fromAmount: baseUnits });
      setExecuteResult({ fromAmount: result.fromAmount, toAmount: result.toAmount, toCurrency: result.toCurrency });
      setSucceeded(true);
    } catch (err: any) {
      Alert.alert('Swap Failed', err?.message ?? 'Something went wrong. Please try again.');
    }
        ...
        <Box flex={1} alignItems="center" justifyContent="center" gap="xl" paddingHorizontal="2xl">
          ...
          <Box alignItems="center" gap="s">
            ...
            <Text variant="body" color="textSecondary" style={styles.centered}>
              {formatBaseUnits(executeResult.fromAmount, from)} {from} → {formatBaseUnits(executeResult.toAmount, executeResult.toCurrency)} {executeResult.toCurrency}
            </Text>
          </Box>
          <Box style={{ width: '100%' }} gap="m">
            ...
            <Button
              label="Swap Again"
              variant="secondary"
              onPress={() => { setSucceeded(false); setAmount('0'); setExecuteResult(null); }}
            />
          </Box>
        </Box>
      </View>
    ...
    <View style={[styles.screen, { backgroundColor: colors.bgPrimary, paddingTop: insets.top + 8 }]}>
      ...
      >
        ...
        <Box gap="xs">
          ...
          <Text variant="label" color="textTertiary">Savings wallet · USDC ↔ EURC</Text>
        </Box>
      </Box>
      ...
      >
        <Box gap="xs" flex={1}>
          <Text variant="caption" color="textTertiary">You give</Text>
        ...
        >
          <ArrowSwapHorizontal size={20} color={colors.textPrimary} variant="Linear" />
        </Pressable>

        <Box gap="xs" flex={1} alignItems="flex-end">
          <Text variant="caption" color="textTertiary">You get</Text>
      ...
      >
        ...
        <Box flexDirection="row" justifyContent="space-between">
          <Text variant="caption" color="textTertiary">Rate</Text>
          <Text variant="captionMedium">
            {previewData ? `1 ${from} = ${previewData.rate} ${to}` : '–'}
          </Text>
        </Box>
        <Box flexDirection="row" justifyContent="space-between">
          <Text variant="caption" color="textTertiary">Fee</Text>
